@framework @T-02 @T-01
Feature: /aa-extend helps the user create extensions to the framework
  The core roughs in the framework and expects others to supply the specifics (T-02). /aa-extend
  ...
  Scenario: Scaffold a project skill
    ...
    And "/aa-health" reports it as covering the technology or concern it names
  ...
  Scenario: An extension declares its requirements
    When "/aa-extend" scaffolds any extension
    Then every capability the extension depends on is declared as a requirement in its own numbered range
    And "/aa-health" aggregates those requirements once the extension is installed
  ...
  Scenario: Validate before installing
    When "/aa-extend" finishes scaffolding
  ...
  Scenario: Share an extension with a team or organisation
    ...
    When I ask "/aa-extend" to share it
  ...
  Scenario: Extend an existing extension
    ...
    When I run "/aa-extend" and choose to add to it
