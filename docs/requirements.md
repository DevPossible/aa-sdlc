...
| R-23 | Every ticket that carries a size carries an implementation plan recorded no later than the size, and the size names the plan as its basis. | required | O-10, G-28, `refine-ticket`, `plan-implementation`, `plan-iteration` | A sample of sized tickets in the configured project each have an implementation plan on the ticket, recorded no later than the size, and the size entry cites it. | Nothing to lay down. `/aa-fw-health` lists sized tickets with no plan; the user plans and re-sizes them, or clears the size. |
