...
| R-18 | The repository follows the conventional folder structure: documents, features, scripts, source (one subfolder per project), tests, and root scripts. | required | O-05, `setup-environment`, `/aa-init` | Each conventional folder exists at the root, or the project config maps its equivalent. | `aa init` creates the missing folders; `/aa-init` proposes mappings for an existing layout. |
