...
know that before `aa setup` than after. The opinions are grouped by what they govern so they can
be read a group at a time. Each has a stable ID, states the alternatives it rejects, and says
what would change the framework's mind. The lead sentence of each group is the short form; the
README repeats those and nothing more.

Together the opinions define what the framework treats as the truth for each kind of
information. The tooling keeps the links between them current (T-12); no one of them holds
another's truth.
...
| Why, and what was decided | the knowledge repository (O-04) and its decision records (O-18) |
---
...
What must be in it is O-16; who makes the commit, and how large, is O-17.
...
Every repository has the same top-level layout, the same four root scripts, the same three
test tiers, and holds everything needed to build and operate the software. A fresh clone plus
the documented secrets is enough. This is what `aa init` lays down and `/aa-fw-health` checks.
...
Requirements are Gherkin scenarios in the repository and the source of truth for what the
...
The history is data: every commit is a Conventional Commit, small and cohesive, made by the
user, traceable to its purpose, and dependency changes go through the package manager so the
history is honest about what was resolved.
...
commit (G-39) is what makes the type honest: a commit that cannot name its type is two commits.
...
*Requirements:* R-01, R-05, R-31. *Guidance:* G-39, G-40.
...
Everything the pipeline does runs locally, the end-to-end tier runs against containers, one
built artifact is promoted through every environment, and what varies by environment is kept
apart from what does not.
...
Development proves the requirement and Testing goes beyond it; decisions are records;
conventions are enforced by tools; tests are deterministic and independent.
...
*Requirements:* R-02, R-03, R-06, R-32. *Guidance:* G-41.
