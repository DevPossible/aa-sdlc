...
- The workflow page is **Methodology** (`methodology.html`), generated in full from the workflow
  ...
  record 0008).
- Navigation is Home, Methodology, Business Case, GitHub. GitHub links the mirror (section 12);
...
Rows are immutable once written (row 56, O-18). A change of mind is a new row that names the
row it supersedes; the superseded row gains only a "Superseded by" note.
...
| 45 | 2026-09-21 | Opinion O-16: the repository holds everything needed to build, deploy, and operate the software: configuration templates without secret values, migrations, automation, and operating documentation; the test is a fresh clone plus the documented secrets (R-30, G-37, G-38) | A repository that builds but cannot be deployed or operated without a file on someone's machine or a wiki page is an output that cannot be reproduced; versioning it all gives every change to how the system runs a history, a review, and a way back |
| 46 | 2026-09-21 | Opinion O-17: commits are small and cohesive, and the agent never commits unless the user asked for that commit; it stages, writes the message, presents, and stops. The framework's own authoring skills now end by staging, not committing (R-31, G-39, G-40); promoted from the candidates list | Review before the commit is the primary point at which a person directs the work; an agent that commits unasked moves review to after the fact and makes the history a record of what it did rather than what the user accepted (T-06) |
| 47 | 2026-09-21 | Opinion O-18: every significant decision, technical, product, or process, is a decision record: one screen, numbered in one sequence per repository, dated, with context, options, decision, and consequences, immutable once accepted and superseded by a new record rather than edited (R-32, G-41); promoted from the candidates list, which is now empty | A record is the smallest unit of "why" that can be found, cited, and trusted; recording the rejected options stops them being re-litigated, and extending the form beyond architecture gives product and process decisions the same standing |
| 48 | 2026-09-21 | Opinion O-19: every change traces back to its purpose through a chain that runs both ways: commit to ticket, ticket to scenario or decision record or page, scenario to ticket; a change that cannot name its purpose is a ticket to create first or work not to do (R-33, G-42) | The links already existed in G-09, G-19, G-26, and T-12; stating the chain as an opinion makes it checkable end to end and stops "while I was in there" changes, which agents produce readily unless every hunk is asked what it is for |
| 49 | 2026-09-21 | Opinion O-20: the design is the simplest one that meets the scenarios that exist; no speculative feature, abstraction, extension point, or option; every component and boundary names the scenario that requires it or the decision record that justifies it (R-34, G-43) | A design built for requirements that do not exist yet is built with the least information and paid for now; agents in particular generalise unasked, and a design beyond the scenarios has no truth to be checked against (T-07, O-19) |
| 50 | 2026-09-21 | Opinion O-21: conventions live as formatter and linter configuration in the repository; formatting runs on changed files, linting runs from the root build and the pipeline; a formatter is required (R-09), a linter recommended (R-12) with health warning where none is known (R-35, G-44) | Conventions in a document are followed by whoever read it; conventions in tool configuration are followed by everyone, including agents, and free review for correctness and the requirement |
| 51 | 2026-09-21 | Opinion O-22: dependencies are added, updated, and removed only through the ecosystem's package manager; a version in a manifest or lock file is never edited by hand, a tool refusal is a finding on the ticket, and manifest and lock changes are committed together (R-36, G-45) | A hand edit breaks the manifest-to-lock relationship silently and skips conflict resolution, warnings, and transitive re-resolution; agents do it readily because it looks like the smallest change, and it is the largest one made blind |
| 52 | 2026-09-21 | Opinion O-23: every test is deterministic and independent; time, randomness, shared state, and external dependencies are controlled; a test runs alone or in any order with the same result; a flaky test is quarantined with a ticket, never retried (R-37, G-46) | A suite is evidence only while its results mean something; one flaky test teaches everyone to re-run, after which a real failure looks like noise. The four sources of variance each have a known control, and agents reach for sleeps, live calls, and retries unless told not to |
| 53 | 2026-09-21 | Opinion O-24: a release artifact is built once per commit with an immutable identity, and every environment deploys that identity; configuration is supplied at deploy time from committed templates, nothing is rebuilt for a later environment, and validation confirms the running identity (R-38, G-47); promoted from an inline rule in `setup-pipeline` | Testing an artifact in one environment tells you about the next only if it is the same artifact; a rebuild breaks that inference however reproducible the build claims to be, and one identity makes rollback exact and "what is running" checkable |
...
- **Public hosting and licence.** Closed 2026-09-22. GitLab is the origin and runs the
  pipeline; GitLab push mirrors copy protected branches to the public
  `github.com/DevPossible/aa-sdlc` and every branch to the Azure DevOps `OpenSource` project
  as a backup, the same arrangement as the other open-source repositories. The licence is
  FSL-1.1-ALv2 with AA-SDLC as a DevPossible trademark (decision record
  [0009](decisions/0009-functional-source-license.md)).
